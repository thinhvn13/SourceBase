...
namespace WebApi01.Services
{
    public interface IUserService
    {
        ...
        IEnumerable<User> GetAll(int curId);
        User GetById(int curId, int id);
    ...
    public class UserService : IUserService
    {
        ...
        public IEnumerable<User> GetAll(int curId)
        {
            var user = _context.Users.Find(curId);
            if (!user.is_admin)
            {
                throw new AppException("You are not given access to that information");
            }
            return _context.Users;
        }

        public User GetById(int curId, int id)
        {
            if (curId != id)
                throw new AppException("You are not given access to that information");
